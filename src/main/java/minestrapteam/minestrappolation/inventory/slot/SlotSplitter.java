...
public class SlotSplitter extends Slot
{
	private EntityPlayer	thePlayer;
	private int				amountCrafted;
	
	public SlotSplitter(EntityPlayer player, IInventory inventory, int id, int x, int y)
	{
	...
	protected void onCrafting(ItemStack stack)
	{
		...
		if (!this.thePlayer.worldObj.isRemote)
		{
			...
			float exp = SplitterRecipes.instance().getSmeltingExperience(stack);
			int exp1;
			if (exp == 0.0F)
			{
				amount = 0;
