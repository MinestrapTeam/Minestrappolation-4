...
public class ItemChargeGun extends Item
{
	...
	public void onPlayerStoppedUsing(ItemStack stack, World worldIn, EntityPlayer playerIn, int timeLeft)
	{
		...
		net.minecraftforge.event.entity.player.ArrowLooseEvent event = new net.minecraftforge.event.entity.player.ArrowLooseEvent(playerIn,
		                                                                                                                          stack,
		                                                                                                                          j);
		...
		if (flag || playerIn.inventory.hasItem(Items.fire_charge) || playerIn.inventory.hasItem(MItems.ice_charge))
		{
			if (playerIn.inventory.hasItem(Items.fire_charge))
				this.ammo = Items.fire_charge;
			else if (playerIn.inventory.hasItem(MItems.ice_charge))
				this.ammo = MItems.ice_charge;
			...
			double accelX = (double) (-MathHelper.sin(playerIn.rotationYaw / 180.0F * (float) Math.PI) * MathHelper.cos(
				playerIn.rotationPitch / 180.0F * (float) Math.PI));
			double accelY = (double) (-MathHelper.sin(playerIn.rotationPitch / 180.0F * (float) Math.PI));
			double accelZ = (double) (MathHelper.cos(playerIn.rotationYaw / 180.0F * (float) Math.PI) * MathHelper.cos(
				playerIn.rotationPitch / 180.0F * (float) Math.PI));
			...
			entityFireCharge
				.setLocationAndAngles(playerIn.posX, playerIn.posY + (double) (playerIn.height / 2.0F), playerIn.posZ,
			...
			entityIceCharge
				.setLocationAndAngles(playerIn.posX, playerIn.posY + (double) (playerIn.height / 2.0F + 0.5F),
				                      playerIn.posZ, playerIn.rotationYaw, playerIn.rotationPitch);
			...
			worldIn.playAuxSFXAtEntity(playerIn, 1009,
			                           new BlockPos((int) playerIn.posX, (int) playerIn.posY, (int) playerIn.posZ), 0);
			...
			if (!worldIn.isRemote)
			{
				if (this.ammo == Items.fire_charge)
					worldIn.spawnEntityInWorld(entityFireCharge);
				else if (this.ammo == MItems.ice_charge)
	...
	{
		net.minecraftforge.event.entity.player.ArrowNockEvent event = new net.minecraftforge.event.entity.player.ArrowNockEvent(playerIn,
		                                                                                                                        itemStackIn);
		if (net.minecraftforge.common.MinecraftForge.EVENT_BUS.post(event))
			return event.result;

		if (playerIn.capabilities.isCreativeMode || playerIn.inventory.hasItem(Items.fire_charge) || playerIn.inventory
			                                                                                             .hasItem(
				                                                                                             MItems.ice_charge))
		{
