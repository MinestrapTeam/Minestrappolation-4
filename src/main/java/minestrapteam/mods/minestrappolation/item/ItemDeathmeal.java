...
public class ItemDeathmeal extends Item
{
	...
	public static int isOrganic(IBlockState iblockstate)
	{
		...
		else if (iblockstate.getBlock() == Blocks.deadbush ||
		...
		{
			return iblockstate.getValue(BlockCrops.AGE).intValue() + 1;
		}
		else if (iblockstate.getBlock() instanceof BlockBush)
		{
			return iblockstate.getValue(BlockBush.AGE).intValue() + 1;
		}
		else
		{
			return 0;
	...
	public static boolean applyBonemeal(ItemStack stack, World worldIn, BlockPos target, EntityPlayer player)
	{
		...
		if (isOrganic(iblockstate) != 0)
		{
			...
			{
				worldIn.setBlockState(pos1, MBlocks.wither_ash.getDefaultState().withProperty(BlockWitherAsh.LAYERS,
				                                                                              isOrganic(worldIn
					                                                                                        .getBlockState(
						                                                                                        pos1))));
				dir = rand.nextInt(6);
