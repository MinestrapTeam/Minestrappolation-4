package minestrapteam.mods.minestrappolation.lib;

import java.util.Arrays;
import java.util.List;

public class MReference
{
	public static final String			MODID				= "ministrapp";
	public static final String			ACRONYM				= "M";
	public static final String			NAME				= "Minestrappolation Mod";
	public static final String			VERSION				= "1.8.0-4.2.666";
	
	public static final List<String>	AUTHORS				= Arrays.asList("Delocuro", "TheWerty1124", "SoBioHazardous");
	
	public static final String			CLIENT_PROXY_CLASS	= "minestrapteam.mods.minestrappolation.client.ClientProxy";
	public static final String			SERVER_PROXY_CLASS	= "minestrapteam.mods.minestrappolation.network.CommonProxy";
}
