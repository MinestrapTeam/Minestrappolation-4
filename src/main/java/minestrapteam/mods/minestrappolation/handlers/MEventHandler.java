...
public class MEventHandler
{
	...
	{
		VersionChecker check = new VersionChecker(MReference.VERSION,
		                                          "https://raw.githubusercontent.com/MinestrapTeam/Minestrappolation-4/master/version.txt",
		                                          MReference.NAME);
	...
	public void onBlockBreak(BlockEvent.BreakEvent event)
	{
		...
		if (event.state.getBlock() instanceof BlockNetherWart)
		{
			...
			if (rand.nextInt(100) < Config.soulGemDropChance && event.state.getValue(BlockNetherWart.AGE) == 3
				    && ground.getBlock() == MBlocks.soul_ore)
			{
		...
		if (event.state.getBlock() == Blocks.double_plant)
		{
			if (event.state.getValue(BlockDoublePlant.VARIANT) == BlockDoublePlant.EnumPlantType.GRASS)
			{
				...
				EntityItem eitem = new EntityItem(event.world, event.pos.getX(), event.pos.getY(), event.pos.getZ(),
				                                  item);
	...
	public void onBlockHarvest(BlockEvent.HarvestDropsEvent event)
	{
		...
		if (player != null)
		{
			if (player.getHeldItem() != null && (player.getHeldItem().getItem() == MItems.fire_pickaxe
				                                     || player.getHeldItem().getItem() == MItems.fire_axe
				                                     || player.getHeldItem().getItem() == MItems.fire_shovel))
			{
				if (FurnaceRecipes.instance().getSmeltingResult(new ItemStack(event.state.getBlock())) != null
					    && FurnaceRecipes.instance().getSmeltingResult(new ItemStack(event.state.getBlock())).getItem()
						       != event.drops.get(0).getItem())
				{
					if ((event.state.getBlock().getHarvestTool(event.state) == "pickaxe"
						     && player.getHeldItem().getItem() == MItems.fire_pickaxe) || (event.state.getBlock().getHarvestTool(
						                                                                                  event.state)
							                                                                   == "axe" &&
							                                                                   player.getHeldItem()
							                                                                         .getItem()
								                                                                   == MItems.fire_axe)
						    || (event.state.getBlock().getHarvestTool(event.state) == "shovel"
							        && player.getHeldItem().getItem() == MItems.fire_shovel))
					{
						ItemStack stack = FurnaceRecipes.instance()
						                                .getSmeltingResult(new ItemStack(event.state.getBlock()));
						event.drops.clear();
	...
	public void playerUpdate(LivingUpdateEvent event)
	{
		if (event.entity instanceof EntityPlayer)
		{
			...
			if (PlayerHelper.hasArmorSet(player, MItems.meurodite_helmet, MItems.meurodite_chestplate,
			                             MItems.meurodite_leggings, MItems.meurodite_boots))
			{
				...
				if (player.isInLava() || player.isBurning())
				{
					if (player.worldObj.rand.nextInt(100) == 1 && !player.capabilities.isCreativeMode)
						player.inventory.damageArmor(1);
				         ...
				         .hasArmorSet(player, MItems.torite_helmet, MItems.torite_chestplate, MItems.torite_leggings,
				                      MItems.torite_boots))
			{
				player.addPotionEffect(new PotionEffect(Potion.regeneration.id, 2, 0, true, false));
			}
			else if (PlayerHelper.hasArmorSet(player, MItems.titanium_helmet, MItems.titanium_chestplate,
			                                  MItems.titanium_leggings, MItems.titanium_boots))
			{
				player.addPotionEffect(new PotionEffect(Potion.resistance.id, 2, 1, true, false));
			}
			else if (PlayerHelper.hasArmorSet(player, MItems.ice_helmet, MItems.ice_chestplate, MItems.ice_leggings,
			                                  MItems.ice_boots))
			{
	...
	public void onDamage(LivingHurtEvent event)
	{
		if (event.entityLiving instanceof EntityPlayer)
		{
			...
			if ("fall".equals(event.source.damageType))
			{
				if (player.inventory.hasItem(MItems.amuletPullum))
				{
					...
					if (stack.getItemDamage() >= stack.getMaxDamage()
						    || stack.getItemDamage() + (int) event.ammount > stack.getMaxDamage())
					{
			...
			{
				if (("mob".equals(event.source.damageType) || "player".equals(event.source.damageType)) && PlayerHelper
					                                                                                           .hasArmorSet(
						                                                                                           player,
						                                                                                           MItems.blazium_helmet,
						                                                                                           MItems.blazium_chestplate,
						                                                                                           MItems.blazium_leggings,
						                                                                                           MItems.blazium_boots))
				{
					player.addPotionEffect(new PotionEffect(Potion.fireResistance.id, 2, 0, true, false));
