...
public class BlockTerraCreep extends Block
{
	...
	public void tryReplaceBlock(World worldIn, BlockPos pos, BlockPos originPos, IBlockState state, Random rand)
	{
		...
		else if (worldIn.getBlockState(pos) == MBlocks.biome_stones.getStateFromMeta(0)
			         || worldIn.getBlockState(pos) == MBlocks.biome_stones.getStateFromMeta(1)
			         || worldIn.getBlockState(pos) == MBlocks.biome_stones.getStateFromMeta(2)
			         || worldIn.getBlockState(pos) == MBlocks.biome_stones.getStateFromMeta(3)
			         || worldIn.getBlockState(pos) == MBlocks.biome_stones.getStateFromMeta(4)
			         || worldIn.getBlockState(pos) == MBlocks.biome_stones.getStateFromMeta(5)
			         || worldIn.getBlockState(pos) == MBlocks.biome_stones.getStateFromMeta(6)
			         || worldIn.getBlockState(pos) == MBlocks.biome_stones.getStateFromMeta(7)
			         || worldIn.getBlockState(pos) == MBlocks.biome_stones.getStateFromMeta(8))
		{
	...
	public Item getItemDropped(IBlockState state, Random rand, int fortune)
	{
		if (((MCreepType) state.getValue(VARIANT)).getMetadata() < 9)
			return Item.getItemFromBlock(
				MBlocks.biome_cobble.getStateFromMeta(this.getMetaFromState(state)).getBlock());
	...
	public float getBlockHardness(World worldIn, BlockPos pos)
	{
		...
		if (state == this.getStateFromMeta(MCreepType.DEEPSTONE.getMetadata()) || state == this.getStateFromMeta(
			MCreepType.DEEPCOLDSTONE.getMetadata()) || state == this.getStateFromMeta(
			MCreepType.DEEPREDROCK.getMetadata()) || state == this.getStateFromMeta(
			MCreepType.GLACIERSTONE.getMetadata()) || state == this.getStateFromMeta(
			MCreepType.POCEANSTONE.getMetadata()))
