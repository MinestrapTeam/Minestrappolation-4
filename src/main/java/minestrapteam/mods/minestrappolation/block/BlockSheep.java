	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumFacing side, float hitX, float hitY, float hitZ)
	{
		...
		if (state.getValue(AGE) == 5 && playerIn.getCurrentEquippedItem().getItem() == Items.shears)
		{
			...
			EntityItem ei = new EntityItem(worldIn, playerIn.posX, playerIn.posY, playerIn.posZ,
			                               new ItemStack(Blocks.wool, quantity, this.dropMeta));
		...
		else if (playerIn.getCurrentEquippedItem() != null && playerIn.getCurrentEquippedItem().getItem() == Items.dye)
		{
			if (playerIn.getCurrentEquippedItem().getItemDamage() == 0)
				...
				worldIn
					.setBlockState(pos, MBlocks.block_sheep_purple.getStateFromMeta(this.getMetaFromState(state)), 2);
				...
				worldIn
					.setBlockState(pos, MBlocks.block_sheep_silver.getStateFromMeta(this.getMetaFromState(state)), 2);
				...
				worldIn
					.setBlockState(pos, MBlocks.block_sheep_yellow.getStateFromMeta(this.getMetaFromState(state)), 2);
			else if (playerIn.getCurrentEquippedItem().getItemDamage() == 12)
				worldIn
					.setBlockState(pos, MBlocks.block_sheep_light_blue.getStateFromMeta(this.getMetaFromState(state)),
					               2);
			else if (playerIn.getCurrentEquippedItem().getItemDamage() == 13)
				worldIn
					.setBlockState(pos, MBlocks.block_sheep_magenta.getStateFromMeta(this.getMetaFromState(state)), 2);
			else if (playerIn.getCurrentEquippedItem().getItemDamage() == 14)
				worldIn
					.setBlockState(pos, MBlocks.block_sheep_orange.getStateFromMeta(this.getMetaFromState(state)), 2);
